  ...
  try {
   ...
   // console.log("Lobbies recebidos:", LobbyExistente); // Log para verificar os lobbies recebidos
    ...
    if (LobbyExistente) {
      ...
      const playerSlots = LobbyExistente.playerSlots;
      const cartas = LobbyExistente.cartas;
      const territorios = LobbyExistente.territorios;

      return { id, leaderId, lobbyName, fase, turno, cartas, playerSlots, territorios };
...
let Dominou = false;
  ...
  async function DestribuirCarta(lobbyId){
   ...
   const PartidaDados = await dadospartida(lobbyId); // Aguarda a resolução da Promise e obtém os dados do lobby
    ...
    console.log(JSON.stringify(  PartidaDados.turno, null, 2)); // Exibe o objeto como uma string JSON formatada
