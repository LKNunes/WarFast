...
// Divide o path em comandos e seus argumentos
const commands = pathData.match(/[A-Za-z][^A-Za-z]*/g);

// Itera sobre os comandos para contar os pontos
commands.forEach(command => {
  // Remove o comando inicial para obter os argumentos
  const args = command.slice(1).trim().split(/[\s,]+/);
  numPoints += args.length / 2; // Cada par de coordenadas é um ponto
});

console.log(`O path SVG tem ${numPoints} pontos.`);
